  ...
  onModelChange
    ...
    <div className="max-w-7xl mx-auto flex flex-col gap-2 sm:gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
        ...
        <Badge variant={backendStatus === 'connected' ? 'default' : 'destructive'} className="w-fit self-start sm:self-auto text-xs sm:text-sm py-0.5 px-2">
      ...
      <div className="flex flex-col xs:flex-row sm:flex-row items-stretch sm:items-center gap-1 sm:gap-3 w-full sm:w-auto">
        ...
        <Select value={selectedModel} onValueChange={onModelChange}>
          <SelectTrigger className="
              ...
              ">
            <span>
              {selectedModel ? emotionModels.find(m => m.value === selectedModel)?.label : "Select the model"}
            </span>
          </SelectTrigger>
          <SelectContent>
            {emotionModels.map(m => <SelectItem key={m.value} value={m.value} className="
                  text-xs sm:text-sm
